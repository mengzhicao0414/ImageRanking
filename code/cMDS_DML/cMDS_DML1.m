...
function[MAE,num_correct_full,num_correct_round,time]=cMDS_DML1(X_train,r_train,X_test,r_test,K2,alpha,T)
t0=cputime;
% Step 1
[Y1,Y,D]=MDS1(X_train,r_train);
% Step 2
% [L]=KNN(X_train,r_train,K2,Y,alpha,T);
[d_test,n_test]=size(X_test);
for i=1:n_test
    X_test0=X_test(:,i);
[B]=KN(X_test0,X_train,D);
